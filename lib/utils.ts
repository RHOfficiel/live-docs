...
export const dateConverter = (timestamp: string): string => {
  ...
  switch (true) {
    case diffInDays > 7:
      return `${Math.floor(diffInDays / 7)} uger siden`;
    case diffInDays >= 1 && diffInDays <= 7:
      return `${Math.floor(diffInDays)} dage siden`;
    case diffInHours >= 1:
      return diffInHours >= 2
        ? `${Math.floor(diffInHours)} timer siden`
        : '1 time siden';
    case diffInMinutes >= 1:
      return diffInMinutes >= 2
        ? `${Math.floor(diffInMinutes)} minutter siden`
        : '1 minut siden';
    default:
      return 'lige nu';
  }
